...
export default function Post() {
  ...
  const [skeletonPost, setSkeletonPost] = useState<boolean>(false);
  ...
  const toggleComments = (index:number) => {
    const updatedVisibleComments = [...visibleComments];
  ...
  const fetchDataPost = async () =>{
    try{
      ...
      setSkeletonPost(true)
    }catch{
      setSkeletonPost(false)
      console.error("error");
    }    
  ...
  return (
    <div className=" max-md:col-span-4 max-lg:col-span-4 col-span-3 col-start-2 max-xl:col-start-1">
      ...
      <section className="w-full flex flex-col  items-center  ">
        {skeletonPost ? 
        <>
        {filteredData.length != 0  ? filteredData && filteredData.map((post, index) => (
          <article key={index} className=" border-[1px] max-md:p-4  border-slate-600 shadow-md shadow-slate-800 hover:shadow-slate-700 hover:border-slate-300 max-lg:w-full transition-all duration-300 py-4 px-6 bg-slate-800 h-min-72 w-full my-4 flex flex-col justify-between rounded-lg">
            <Cards loadComment={loadComment} id={post.id} name={post.name} datePosts={post.datePosts} type={post.type} content={post.content} title={post.title} picture={post.picture}>
              <div className="w-auto flex items-center justify-end">
                ...
                <FaCommentAlt/> Comentarios ({post.comments.length})</p>
              </div>
              <hr className="my-4" />
            </Cards>
            {visibleComments[index] && (
              <ol className="">
                {post.comments.length !== 0 ? (
                  post.comments.map((commment, index) => (
                    <CommentUser
                      key={commment.idComment} 
                      dataComment={commment}
                      deleteComment={deleteComment}
                      />
                  ))
                ) : (
                  <p className="text-zinc-100 text-center mt-2">No hay comentarios</p>
                )}
              </ol>
            )}
          </article>
