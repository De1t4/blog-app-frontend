  ...
  return (
    <nav id='inicio' className=' relative  m-auto text-white items-center bg-slate-950 flex justify-between px-8 h-16'>
      <div className="flex justify-between items-center w-[80rem] m-auto">
        <Link href="/#" className={`${tangerine.className} text-[2.25rem] cursor-pointer font-bold text-2xl`}>
          Digital Corner
        </Link>
        <div className='flex gap-4 font-semibold text-[#F3BF3A]'>
          ...
          {authTokens &&
            <div className="flex gap-4 max-[800px]:hidden">
              <Link href={`/PostCreate`} className='cursor-pointer p-2 rounded-lg hover:bg-slate-800 transition-all duration-300 hover:brightness-105 flex items-center gap-x-1'>
                <FaPlus /> Crear Post
              </Link>
              <Link href={`/Favorites/${authTokens?.idUser}`} className='cursor-pointer p-2 rounded-lg hover:bg-slate-800 transition-all duration-300 hover:brightness-105 flex items-center gap-x-1'>
                <FaHeartCirclePlus />Favoritos
              </Link>
              <p className='cursor-pointer  bg-gold  p-2 rounded-lg text-slate-950 font-bold  transition-all duration-300 hover:brightness-105 flex items-center gap-x-1' onClick={logoutUser}>
